/**
 * PHASETIPS is a tooltip plugin for Phaser.io HTML5 this.game framework
 *
 * COPYRIGHT-2015
...
var Phasetips = function (options) {
    ...
    this.onHoverOver = function () {
        ...
        if (_options.animation === "fade") {
            _this.tweenObj = this.this.game.add.tween(_this.mainGroup).to({
                alpha: 1
            ...
            _this.tweenObj = this.game.add.tween(_this.mainGroup.scale).to({
                x: 1,
                y: 1
            }, _options.animationSpeedShow, Phaser.Easing.Linear.None, true, _options.animationDelay, 0, false);
        }
    ...
    this.onHoverOut = function () {
        ...
        if (_options.animation === "fade") {
            _this.tweenObj = this.game.add.tween(_this.mainGroup).to({
                alpha: 0
    ...
    this.createTooltips = function () {
        ...
        var _object = _options.targetObject || this.game; // any object
        ...
        _this.mainGroup = this.game.add.group();
        ...
        if (type === "string") {
            tooltipContent = new Phaser.Text(this.this.game, _padding / 2, _padding / 2, _content, _textStyle);
        ...
        if (_x !== "auto" && _y !== "auto") {
            ...
            // sanity check
            if (_position === "bottom") {
                if (Math.round(_object.y + _object.height + (_positionOffset)) + mainGroup._height > this.game.height) {
                    _position = "top";
                }
        ...
        if (_customBackground === false) {
            /// create bg
            tooltipBG = this.game.add.graphics(tooltipContent.width, tooltipContent.height);
